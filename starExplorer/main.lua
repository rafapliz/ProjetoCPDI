...
local audioExplosion = audio.loadSound ("audio/explosion.wav")
...
math.randomseed (os.time())-- Faz com que os mathrandoms não tenham um padrão previsível.
local gameLoopTimer
local morto = false
...
local function onCollision (event)
    if (event.phase == "began") then
        ...
        (obj1.myName == "Asteroide" and obj2.myName == "Laser")) then
            audio.play (audioExplosion, parametros)
            display.remove (obj1)
            display.remove (obj2)

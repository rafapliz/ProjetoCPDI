...
-- --Criação da junta de pivô ("tipo de junta", objA, objB, âncoraX, âncoraY)
...
-- jointPivot.maxMotorTorque = 1000 -- Define o valor máximo da velociade de torque do motor, utilizado para melhor visualização do efeito.
...
-- jointDistance.dampingRatio = 0.8 -- Nível de amortecimento, onde 1 é o amortecimento crítico.
...
-- local staticBox = display.newRect (0, 0, 60, 60)
-- staticBox:setFillColor (0.2, 0.2, 1)
-- physics.addBody (staticBox, "static", {ifSencor=true})
...
-- -- Criação da junta de pistão ("tipo de junta, abjA, objB, ancoraX, ancoray, eixo de movimentaçãoX, eixo de movimentaçãoY")
...
-- jointPiston.motorSpeed = -30 -- Define a velocidade da junta (valor negativo faz a movimentação inversa)
...
-- transition.to (bodiesGroup, {tome=600, alpha=1, transition=easing.outQuad}) -- Transição do grupo para o alfa 1 em 600 milisegundos com efeito outQuad.
...
-- -- Atribui a frequência e damping para a junta dos valores já criados anteriormente
